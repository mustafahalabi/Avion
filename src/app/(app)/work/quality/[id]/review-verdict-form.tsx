...
import type { ReviewFinding } from "@/lib/review-service";
...
export function ReviewVerdictForm({ reviewId }: Props) {
  ...
  function addFinding() {
    ...
    setFindings((prev) => [
      ...prev,
      { severity: newFindingSeverity, description: desc, actionable: newFindingSeverity === "blocker" },
    ]);
    setNewFinding("");
  }
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (verdict === "changes_requested" && !notes.trim() && findings.length === 0) {
      setError("Describe the changes needed or add at least one finding.");
      return;
    }
    if (verdict === "blocked" && !notes.trim()) {
      setError("Describe why the review is blocked.");
      return;
    }
    setError(null);
    startTransition(async () => {
      await submitReviewVerdict(reviewId, verdict, notes.trim(), findings);
      router.refresh();
    });
  }

  const showFindings =
    verdict === "changes_requested" || verdict === "blocked" || verdict === "needs_clarification";
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-2 flex flex-col gap-4">
      ...
      <textarea
        ...
        placeholder={
          verdict === "approved"
            ...
            ? "Describe the changes needed (or use findings below)…"
            : verdict === "blocked"
            ? "Describe why the review is blocked…"
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex gap-2">
            <select
              ...
              onChange={(e) => setNewFindingSeverity(e.target.value as "blocker" | "non_blocker")}
      ...
      >
        {pending ? "Submitting…" : "Submit Verdict"}
      </button>
    </form>
  );
}
