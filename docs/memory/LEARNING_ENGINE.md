...
- **QA results carry pass/fail evidence and block on failure.** `recordQaResult` (`src/lib/qa-service.ts`) stores QA verdicts and per-check results against acceptance criteria and **blocks completion** when required checks did not pass — no task reaches `done` on an unverified claim.
...
- **The durable memory store exists.** `Memory` / `MemoryRecord` (`src/app/actions/memory.ts`) persist insights with a `source` and a `confidence` value (0–1) across eight categories (`company`, `architecture`, `product`, `security`, `operations`, `employee`, `feature`, `decision`). Five company-scoped banks are seeded at company creation (`src/lib/company-seed.ts`), and the `/memory` surfaces let the CEO browse and add records.
