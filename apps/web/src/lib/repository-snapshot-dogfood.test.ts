...
describe("Repository Intelligence Slice 2 dogfood flow", () => {
  it("persists two real repository analyses, compares them, and generates CEO-readable impact", async () => {
    ...
    // docs/ lives at the monorepo root (not under apps/web), so the fixture copy
    // of process.cwd() (apps/web) won't already contain docs/reviews/ — create it.
    mkdirSync(join(fixturePath, "docs/reviews"), { recursive: true });
    writeFileSync(
