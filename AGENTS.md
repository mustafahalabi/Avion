...
- **Open (Backlog):** the **audit-reality pass (July 2)** filed 19 tickets `MUS-281`–`MUS-299`; the two Urgent (`MUS-281`, `MUS-282`), two P2 residuals (`MUS-284`, `MUS-285`), the P2 loop-integrity trio (`MUS-283`, `MUS-287`, `MUS-286`), and the P2 product-completeness batch (`MUS-288` culture wired, `MUS-289` dead models removed, `MUS-290` board.gateway tested) shipped immediately. The P3 **loop-integrity hardening** then began: `MUS-292` (QA `passed` with zero evidence → now holds for CEO QA). Remaining, by priority — **P3:** `MUS-291` (README Electron drift), `MUS-293` (git no-timeout hang), `MUS-294` (session idempotency race), `MUS-295` (silent sub-autonomous strands); **P4:** `MUS-296` (createTask accepts `done`), `MUS-297` (outcome/feature lifecycles never terminate), `MUS-298` (dead schema/config), `MUS-299` (analyzeRepository arbitrary host-path read). Plus the older `MUS-268` (pgvector recall) and `MUS-276` (Codex host auth). The auth/scoping audit found **no cross-tenant or missing-auth holes**.
...
pnpm --filter @avion/web test         # tsc --noEmit && vitest run (113 files / 1,781 cases)
