...
- **Open (Backlog):** only two remain, both P4 "when it's needed" — `MUS-268` (semantic memory recall via pgvector, once memory volume warrants it) and `MUS-276` (Codex provider connection + worker-host CLI auth before any company flips to the codex agent).
...
pnpm --filter @avion/web test         # tsc --noEmit && vitest run (101 files / 1,683 cases)
...
Tiers 1 and 2 (loop truthfulness + platform health) have shipped and are verified live (see the Status line and MUS-266); the backlog is down to two "when needed" P4 items. What remains:

- **Tier 1 — Make every "it shipped" true.** ✅ **Complete.** Every loop-truthfulness gap the live run surfaced is closed: `MUS-274`/`271`/`277`/`280`/`279`/`278`/`272` (July 2, #104–#112), plus `MUS-270` (driver reworks planless/ad-hoc tasks, #117) and `MUS-273` (live scripts use the memory-carrying prepare path, #118). Nothing open here.
