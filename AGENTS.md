...
- **Open (Backlog):** the **audit-reality pass (July 2)** filed 19 tickets `MUS-281`–`MUS-299`; the two Urgent (`MUS-281`, `MUS-282`) and two P2 residuals (`MUS-284`, `MUS-285`) shipped immediately. Remaining, by priority — **P2:** `MUS-283` (missing agent binary crashes the worker — blocks codex), `MUS-286` (auto-merge ignores commit-status CI), `MUS-287` (a changes-requested review resurrects a `done` task → oscillation), `MUS-288` (`cultureProfile` unwired), `MUS-289` (Subtask/Sprint/Milestone declared, never persisted), `MUS-290` (board.gateway socket auth/scope untested); **P3:** `MUS-291` (README Electron drift), `MUS-292` (QA `passed` with zero evidence), `MUS-293` (git no-timeout hang), `MUS-294` (session idempotency race), `MUS-295` (silent sub-autonomous strands); **P4:** `MUS-296` (createTask accepts `done`), `MUS-297` (outcome/feature lifecycles never terminate), `MUS-298` (dead schema/config), `MUS-299` (analyzeRepository arbitrary host-path read). Plus the older `MUS-268` (pgvector recall) and `MUS-276` (Codex host auth). The auth/scoping audit found **no cross-tenant or missing-auth holes**.
...
pnpm --filter @avion/web test         # tsc --noEmit && vitest run (101 files / 1,691 cases)
...
- **Tier 1 — Make every "it shipped" true.** The live-run gaps are closed (`MUS-274`/`271`/`277`/`280`/`279`/`278`/`272`, `MUS-270`, `MUS-273`) and the audit's two **Urgent** loop-breakers shipped (`MUS-281` guardrail sees committed changes; `MUS-282` real default branch + PR-less-commit fails) along with `MUS-284`/`MUS-285`. Still open from the audit, by leverage: `MUS-283` (agent-binary crash → worker down), `MUS-287` (review resurrects a `done` task → oscillation), `MUS-292` (QA `passed` with no evidence), `MUS-286` (auto-merge ignores commit-status CI), `MUS-293`/`294`/`295` (git-hang / session race / silent strands).
