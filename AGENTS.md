...
- **Open (Backlog):** the **audit-reality pass (July 2)** filed 19 tickets `MUS-281`–`MUS-299`; the two Urgent (`MUS-281`, `MUS-282`), two P2 residuals (`MUS-284`, `MUS-285`), and the P2 loop-integrity trio (`MUS-283`, `MUS-287`, `MUS-286`) shipped immediately. Remaining, by priority — **P2:** `MUS-288` (`cultureProfile` unwired), `MUS-289` (Subtask/Sprint/Milestone declared, never persisted), `MUS-290` (board.gateway socket auth/scope untested); **P3:** `MUS-291` (README Electron drift), `MUS-292` (QA `passed` with zero evidence), `MUS-293` (git no-timeout hang), `MUS-294` (session idempotency race), `MUS-295` (silent sub-autonomous strands); **P4:** `MUS-296` (createTask accepts `done`), `MUS-297` (outcome/feature lifecycles never terminate), `MUS-298` (dead schema/config), `MUS-299` (analyzeRepository arbitrary host-path read). Plus the older `MUS-268` (pgvector recall) and `MUS-276` (Codex host auth). The auth/scoping audit found **no cross-tenant or missing-auth holes**.
...
pnpm --filter @avion/web test         # tsc --noEmit && vitest run (101 files / 1,699 cases)
