...
*(Resolved before PR #84: no api auth, memory→planner-only, lifecycle never completing, no CI, `MUS-225` — closed via MUS-255/265, 258, 259, 256, 225. **Resolved July 2 in this run:** the deterministic plan now ships a real implementation task so a plan actually builds the change (MUS-274, #104); planner provenance is recorded + shown as a badge, so the silent AI→deterministic fallback is now visible (MUS-271, #105); the live `Task.description` reaches the execution brief as an authoritative objective (MUS-277, #106); and orphaned `running` sessions from a dead worker are reaped so they can't stall the driver (MUS-280, #107). The gaps below remain open as of July 2.)*

- **A committing rework that fails QA resets the bounded-retry budget** (MUS-279, P2): the budget counts failed sessions since the last *completed* execution session, and a run that produces a commit ingests as `completed` — so only *no-op* reworks are truly bounded. A rework that keeps committing wrong-but-non-empty fixes can loop without ever escalating to the CEO.
...
- **The driver can't rework ad-hoc tasks** (MUS-270): task selection requires an approved/applied planning draft, so a task with no draft (e.g. a chat- or manually-created one) that gets a change request strands — its rework never re-enters the loop.
- **The adapter `filesChanged` parser can record fake paths** (MUS-278): Implementation-Summary bullets after a `- Files changed:` line are captured as file paths, and the `git diff` fallback only fires on an *empty* parse, so a bogus non-empty parse suppresses it — an audit-trail integrity bug (a real single-file PR recorded four summary-bullet "files").
...
- **Tier 1 — Make every "it shipped" true.** Landed July 2: `MUS-274` (deterministic plan ships a real implementation task), `MUS-271` (planner provenance recorded + shown), `MUS-277` (live task description reaches the brief), `MUS-280` (orphaned `running` sessions reaped). Still open: `MUS-279` (P2 — a committing rework that fails QA resets the retry budget, so only no-op reworks are bounded), `MUS-278` (filesChanged parser records fake paths — audit integrity), `MUS-272` (inapplicable validation commands → false QA failures), `MUS-270` (driver can't rework ad-hoc tasks), `MUS-273` (live scripts bypass the memory-carrying prepare path).
