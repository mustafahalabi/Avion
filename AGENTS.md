...
- **The execution worker can now be sandboxed from the host** (Goal 1, shipped): set `WORKER_SANDBOX=docker` and every host-shell surface of a session (the agent CLI + the install/validation shells) runs inside an ephemeral `docker run --rm` container that bind-mounts **only** the checkout — its own PID namespace means the agent cannot see or SIGTERM host processes (verified: `ps` shows a handful of PIDs inside vs the host's hundreds), while edits still land on the host checkout for the host-side git commit/push. Build the image with `docker build -f apps/web/Dockerfile.sandbox -t avion-agent-sandbox:latest .`; tunables are `WORKER_SANDBOX_IMAGE/NETWORK/MEMORY/CPUS/PIDS_LIMIT/USER/ENV/MOUNTS` (see `docs/DEPLOYMENT.md` → "Sandboxing the execution worker"). Default is still `WORKER_SANDBOX=none` (unchanged behavior). Un-sandboxed, the `WORKER_PERMISSION_MODE` cap is now **dev-only**: with it unset a `full` level auto-caps to `execute` for host safety unless `WORKER_ALLOW_UNSANDBOXED_FULL=1`. Seam: `src/lib/adapters/sandbox-runner.ts` (+ `worker-effective-permission.ts`, `worker/sandbox-command-spawn.ts`).
...
pnpm --filter @avion/web supervise    # one command: supervised single-instance worker + driver (Goal 4)
# WORKER_SANDBOX=docker  → isolate the agent in a container (Goal 1; build apps/web/Dockerfile.sandbox)
# WORKER_CONCURRENCY=N   → run N sessions in parallel, each its own sandbox + live feed (Goal 5a)
# EOS_OUTCOME_SPEND_CEILING_USD / CompanySettings.spendCeilingUsd → halt a runaway outcome (Goal 3)
pnpm --filter @avion/web test         # tsc --noEmit && vitest run (139 files / 2,010 cases) — do NOT source .env
...
- **`DATABASE_URL` must be exported** for the worker, driver, and integration tests — they don't auto-load `.env` (e.g. `DATABASE_URL="postgresql://postgres:postgres@localhost:5433/avion" pnpm worker`). In practice **source `apps/web/.env`** before `pnpm worker`/`pnpm driver` (`set -a; . apps/web/.env; set +a`) so they also get `CREDENTIALS_ENCRYPTION_KEY` (needed to decrypt the GitHub token — without it a tokenless clone hangs). The `live:*` scripts source `apps/web/.env` themselves.
- **`WORKER_PERMISSION_MODE` caps the execution agent's permission** (host-safety). Unset → the agent runs at the autonomy-derived level (`autonomous` → `full` → `bypassPermissions` = arbitrary host shell access). Set `WORKER_PERMISSION_MODE=execute` (→ `acceptEdits`) when running the worker un-sandboxed on a real machine so the agent can't run destructive host commands. Remove it / set `full` **only** inside an isolated worker (container/VM). See the worker-not-sandboxed caveat above and `docs/NEXT-STEPS.md` Goal 1.
...
The loop-integrity and platform-health work (the earlier tiers + the whole `MUS-281`–`MUS-299` audit pass) has **shipped and is verified live**. Operating the autonomous loop end-to-end (July 3–4) then revealed that the current frontier is **operational**, not correctness: the loop *works* (plans auto-apply at `autonomous`, PRs open + auto-merge, tasks reach `done`), but it is not yet safe, observable, or robust enough to run unattended. The five next goals are written as **ready-to-hand-off prompts in `docs/NEXT-STEPS.md`** — tackle them in order:

1. **Sandbox the execution worker** *(top priority)* — isolate `claude -p` (container/ephemeral VM) so it can't touch the host and can safely run at full power inside the sandbox. Fixes the dev-server crashes and lets the `WORKER_PERMISSION_MODE=execute` stopgap be removed. See the worker-not-sandboxed caveat above.
2. **Live feedback for the planning phase** — extend the PR #153 live-output stream to the ~1–2 min planning window so the chat isn't silently stuck at "Intake · Product Manager".
3. **Cost + activity observability** — per-outcome spend/token meter, a "what's running right now" ops view, and an optional per-outcome spend ceiling.
4. **Operational hardening** — supervised, health-checked, single-instance worker/driver/dev (wire the MUS-269 deployment slice); reliable orphaned-session reaping.
5. **Product depth** — parallel execution (worker pool, each session with its own live feed), mid-flight chat steering, and semantic memory (`MUS-268` pgvector). Folds in `MUS-276` (Codex host auth). **Do Goal 1 first** — parallel full-permission agents on the host would be catastrophic.
