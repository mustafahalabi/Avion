...
- **Tier 1 — Make every "it shipped" true.** Landed July 2: `MUS-274` (deterministic plan ships a real implementation task), `MUS-271` (planner provenance recorded + shown), `MUS-277` (live task description reaches the brief), `MUS-280` (orphaned `running` sessions reaped), `MUS-279` (retry budget counts failed QA gates, so committing reworks are bounded too). Still open: `MUS-278` (filesChanged parser records fake paths — audit integrity; also `parseFilesChangedFromGit` diffs `HEAD~1`, wrong for the agent's uncommitted changes), `MUS-272` (inapplicable validation commands → false QA failures), `MUS-270` (driver can't rework ad-hoc tasks), `MUS-273` (live scripts bypass the memory-carrying prepare path).
